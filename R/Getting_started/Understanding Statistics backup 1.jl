...
# ╔═╡ 5231780d-7ffd-4880-b259-58e41272b169
using Plots, PlutoUI, DataFrames, CSV, GLM, Statistics, LinearAlgebra, Distributions, Turing, StatsPlots, MCMCChains, DynamicHMC, Random


# ╔═╡ b7548784-5810-11ec-3e74-1f57af890804
md"""
# Modeling and Fitting Functional Response Curves
"""

# ╔═╡ 72626801-9441-4abc-8d9f-c133a9c9b91a
md"""
Functional response is the number of prey successfully attacked per predator as a function of prey density (Solomon, 1949). It describes the way a predator responds to the changing density of its prey. Holling (1959) considered three types of functional response (Fathipour and Maleknia 2016).

	1. Type I: there is a linear relation between prey density and the maximum number of prey killed
	2. Type II: the proportion of prey consumed declines monotonically with 	prey density. 
	3. Type III is described by a sigmoid relation in which the proportion of the prey consumed is positively density-dependent over some regions of prey density. 

Functional response models help to evaluate two vital parameters: handling time (i.e., the time needed to attack, consume, and digest the prey), and attack rate or searching efficiency (i.e., the rate at which a predator searches for finding its prey). 

The functional response can be defined as a consumer's per capita ingestion rate f as a function of resource density *R*, and it is often described by the Hollings formula

$$f(R)=\frac{a \cdot R^{b}}{1 + a \cdot  h \cdot  R^{b}}$$

where $a$ is the consumer's attack rate coefficient and $h$ is its handling time. The Hill exponent $b$ determines the shape of the functional response:

	- if b=1, b yields a saturation type II response
	- if b>1, b yields a sigmoidal type III response

"""

# ╔═╡ 76183dfc-f5d8-48a3-878f-2d599b1639c2
md"""
## Simulating functional response curves
"""

# ╔═╡ 2af4f22a-238e-4da1-97b8-f9ec1105ea68
function f_R(a, h, b, R)
	fR = (a * (R^b))/ (1 + a*h*(R^b))
	return fR
end

# ╔═╡ 814654c8-5da8-4ba5-9bca-7fcd831c49af
md"""
### Test the sensitivity of the functional response to coeficients:
a = $(@bind a Slider(1:1:100, default=2, show_value = true )) Attack rate

h = $(@bind h Slider(0:.05:2, default=0.25, show_value = true )) Handling time

b = $(@bind b Slider(0:.01:3, default= 1, show_value = true )) Hill's exponent

noise = $(@bind noise Slider(0:.01:5,  show_value = true )) Error
"""

# ╔═╡ 84bbb54b-ebc6-4981-aeee-c8b6c83fb3bd
begin
	R = 0:0.01:6
	y = f_R.(a,h,b, R)
	x = R
	plot(x,y, ylim = (0,6))
	yaxis!("Ingestion rate f(R)")
	yy = y .+ (noise * randn(length(y))) 
	sp = rand(1:length(y),100)
	x1 = x[sp]
	y1 = yy[sp]
	scatter!(x1,y1)
end
...
# ╔═╡ 83da3b3c-659e-4191-86ab-5d0f1b455c25
# make a DataFrame
df = DataFrame(Eaten = y1, Resource = x1);

# ╔═╡ 738635b7-66a3-4d64-b470-455aac70e40d
sort!(df, :Resource);

# ╔═╡ 141f9338-b01f-4392-bd61-4fa5817b0556
begin
	scatter(log.(df.Resource), log.(df.Eaten), lab = :false)
	xaxis!("Resource density (ln)")
	yaxis!("Eaten (ln)")
end


# ╔═╡ 30fa73aa-62a6-43f9-a09a-7deb078f3b2f
10^(2)

# ╔═╡ 599a4c67-ca32-4c8c-9a68-da7bb251cf64
log10(100)

# ╔═╡ ed3be4c0-b211-43f1-8b3e-63f6ca365925


# ╔═╡ ae5d4dc0-cb76-4a08-a426-b5a08f42da31


# ╔═╡ 9fea4418-3b72-4d8e-931c-7f12be6160fc


# ╔═╡ eb076636-f4e0-46fa-b439-e4932d43af70
md"""
# Estimating the functional response coeficients
"""

# ╔═╡ fe1362ef-8c34-4760-8b66-052db3962c38
md"""
To model your data your need to define:
	
	1. response variable e.g., f(R)
	2. predictors e.g., R
	3. a model relating your predictors with your response variable

$$f(R)=\frac{a \cdot R^{b}}{1 + a \cdot  h \cdot  R^{b}}$$

	4. a way to estimate the unknown values (parameters) such as Bayesian or frequentist (ML) statistics.
"""

# ╔═╡ 8dec97a9-4031-4022-bded-69c25ec9eb5a
@model function Functional_Response(Eaten, R)
	# unknown parameters with priors
	a ~truncated(Normal(0, 100), 0, Inf)
	b ~ Normal(0,2)
	...
	# fitting the data (likelihood)
	Eaten ~ MvNormal(fR, sqrt(σ²))
end

# ╔═╡ ead4bc9f-7259-4a0e-9c68-f60928ae2ef4
# my_model = Functional_Response(df.Eaten, df.Resource)

# ╔═╡ 4a337291-b47b-4e32-b797-c90c682a7305
 # chain = sample(my_model, drop_warmup = true, NUTS(0.75), MCMCThreads(), 4000, 4);

# ╔═╡ ed8d3d49-c911-4ee6-bbab-f678cc615c75
# plot(chain)

# ╔═╡ 88d1aab4-05cd-4f42-b61c-e63f262e2a2d
# describe(chain)

# ╔═╡ e83d681f-45eb-4d47-b630-ea63a8a7df0c
# post = DataFrame(chain)

# ╔═╡ ab301a80-82eb-41bf-83ae-ecaa24934ac5
md"""
...
# ╔═╡ 7284130b-b9ac-4a6e-bea6-258ea3f48a01
# make a prediction function

function Pred_FR(post, R)
	a = post.a
	b = post.b
	h = post.h
	fR = ( a .* (R.^b))./ (1 .+ a .* h .* (R.^b))
...
# ╔═╡ 4aad91f4-af67-4401-9c6a-d0945dac3257
# Pred_FR(post, 10)

# ╔═╡ 193b271c-ed62-425d-aa82-2a23f94c2d56
#arr = [Pred_FR(post, w) for w in df.Resource]

# ╔═╡ 34ad52b3-05ad-41ef-ae35-43fdcb11dbd3
#raw_p = DataFrame(arr, :auto)

# ╔═╡ 645bd6c8-aa82-4307-8a4d-8c83e9b09135
...
# ╔═╡ 52c127ad-d3a3-4f1c-94f0-2c52a4cf3df8
# begin
# ci = (mapcols(x -> HDI(x, credible_mass=0.95), raw_p))
# # p_val = (mapcols(x -> boot_p(x), Δ13C_net))
# Predictions = DataFrame(parameter = names(raw_p), mean = round.(mean.(eachcol(raw_p)), digits=3),
# lc = round.(Vector(ci[1, :]), digits =3), up = round.(Vector(ci[2, :]), digits =3) );
# end

# ╔═╡ cc72e9c0-0432-49a9-833e-e761799a4724
# begin
# 	scatter(df.Resource, df.Eaten, ylim = (0,4))
# 	plot!(df.Resource, Predictions.mean, ribbon = [Predictions.lc .- Predictions.mean, Predictions.up.-Predictions.mean], legend = false, ylim = (0,6))
# 	yaxis!("Ingestion rate")
# 	xaxis!("Resource density")
...
git-tree-sha1 = "2c54ffd46315e821ef6cf84672832edf45d7683a"
...
# ╟─52c127ad-d3a3-4f1c-94f0-2c52a4cf3df8
# ╠═cc72e9c0-0432-49a9-833e-e761799a4724
# ╟─5cd1d3cf-e91d-4c32-9afe-5a884ff75434
# ╟─c6d4e4af-c16c-484f-aded-013ba667b944
# ╠═94c47090-24c9-468d-89b4-71d602a4575d
# ╠═cd338869-23f1-47f5-8524-0252ac810ec4
# ╟─09ac44d7-8ad6-4bf6-b607-676efc783765
# ╠═0f00e14c-fffc-4296-9291-14f4eeacec78
# ╠═acf490e6-3aed-4e5a-bfbe-3d282fe6351d
# ╠═3715d13e-b4c7-4190-927e-3fb6aa93abbb
# ╠═f6f590be-bf64-424d-8d2f-4b853734cc44
# ╠═26138d95-1688-4165-bd28-0325b1d58982
# ╠═92740c9f-9b14-43fe-b958-ff437a3bc96b
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
