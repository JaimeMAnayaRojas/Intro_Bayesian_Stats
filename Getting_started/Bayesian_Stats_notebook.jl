...
begin
	using CSV # module for reading and writing CSV files
	using Plots # module for plotting
	using StatsPlots
	using DataFrames # ... working with data frames
	using Random # ... random numbers
	using Turing # Module for Bayesian statistics in Julia
...
md"""
## 2. Get your data. 
In this example, we will follow Richard McElreath's example on the partial census of the Dobe are !Kung San. This data was compiled b Nancy Howell in the late 1960s. The!Kung San is the most famous foraging human population of the $20^{th}$ century.
"""

# ╔═╡ 6c1f4059-8bdc-459a-bc70-c6096ee3df76
md"""
![](http://4.bp.blogspot.com/-UW91cSCRxsM/T8be8X2SAEI/AAAAAAAAACQ/4mYrOfVpAlY/s640/bushmen-community-in-central-kalahari-sanpeopleofsouthafrica.jpg)"""

# ╔═╡ c27d568a-f01e-4973-a013-9a55b2ed2955
begin
data = CSV.read("Howell1.csv", DataFrame; delim = ",");
#data = filter!(x -> x.age >= 18, data);
end

# ╔═╡ f3fec651-8bb5-4d03-9e4b-f75a070cba50
Men = filter(x -> x.male == 1, data);

# ╔═╡ 57b24a88-e051-4493-855a-8ebc0ff16147
Women = filter(x -> x.male == 0, data);

# ╔═╡ c15843f9-1f4e-416c-81d6-8575ff125167
begin
	scatter(Women.weight, Women.height, lab= "Women", legend = :topleft)
	scatter!(Men.weight, Men.height, lab= "Men")
	xlabel!("Weight (Kg)")
	ylabel!("Height (cm)")
end

# ╔═╡ 05529e8a-6ee7-42a4-8bff-7997b180b480
begin
	scatter(log.(Women.weight), log.(Women.height), lab= "Women", legend = :topleft)
	scatter!(log.(Men.weight), log.(Men.height), lab= "Men")
	xlabel!("Weight ln(Kg)")
	ylabel!("Height ln(cm)")
end

# ╔═╡ c8d9d676-b097-4d6e-8d4a-de99860dbb33
md"""
## The model
We want to model the relationship between weight and height and test whether this relationship is different between men and women. 

### Our model:
$x_{i}= weight_{i} - \overline{weight}$
$Y \approx Normal(\mu, \sigma)$
$\mu_{i} = \alpha + \beta_{w} x_{i} + \beta_{m} Male_{i} + \beta_{mxw} Male_{i}x_{i}$
$\alpha \approx Normal(0, 10)$
$\beta_{w} \approx Normal(0, 10)$
$\beta_{m} \approx Normal(0, 10)$
$\beta_{mxw} \approx Normal(0, 10)$
$\sigma \approx Cauchy(0, 2)$
"""

# ╔═╡ e230eb0f-e5a1-4684-9908-1a5757581f94
md"""
## Define our model

In Julia, we basically make a function that looks almost exactly as in our writing model definition
"""

# ╔═╡ f5ca06cc-4340-4c84-89d4-9b8f38e63158
begin
density(rand(Normal(4.93, 0.1), 1000))
xlabel!("α priors (log-scale)")
end

# ╔═╡ 2b8ea7d4-cb40-452a-a1a1-273bf3529601
begin
density(exp.(rand(Normal(4.93, 0.1), 1000)))
xlabel!("α priors (natural-scale)")
end

# ╔═╡ 9d8c8efa-6cae-4d89-b5bd-7c9f261047c0
md"""
## Create a model, with your data.
"""

# ╔═╡ fb1a2138-cedf-4f0d-8d43-5ef9b5955f2c
md"""
## Now, we run the model, and collect our posterior samples.
"""

# ╔═╡ b97076e9-cf43-4740-9f67-f505e7ae7ffc
md"""
## Now, we check if the MCMC chains converged
# these are your posteriors
"""

# ╔═╡ 83b5d10b-2ed9-403e-9c68-cdec255d4930
md"""
## Check your summary statistics
"""

# ╔═╡ 26651707-1f74-4606-8ab2-637dfd73fca4
md"""
## Get the posterior predictions of the models and test your hypotheses
"""

# ╔═╡ 6186521e-87c9-42e0-86c3-b6df828ba910
## model from the posteriors
function p_link(weight, male, α, β_w, β_m, β_wm) # weight in Kg, Male==1 female==0, ...
    x = log.(weight) .- log.(45)
	p = α .+ β_w .* x .+ β_m .* male .+ β_wm .* (x.*male)
    return exp.(p)
end


# ╔═╡ b707db9e-5063-406c-ba5c-61f849eb420a
md"""
#### Get summary statistics such as complementary intervals (CI)
"""

# ╔═╡ ab9a72b6-b899-4ad4-9338-2c4f57d4069f
...
# ╔═╡ 98d4d6b0-bc04-4590-8261-bdaf7c4785f1
md"""
## Let's test our hypothesis
### Research hypothesis 1: 
#### the relationship between height and weight is stronger in males.

"""

# ╔═╡ fe2045ed-1f28-4fff-a186-c7b6347ff358
md"""
Our hypothesis test says that there is a 65.4% posterior probability that the relationship between body weight and height is stronger in men
"""

# ╔═╡ 7cc195a7-4183-4b94-ab6b-0da091ffc116
md"""
### Research Question 1: 
#### What is the predicted height of a 58.0 Kg woman of the !Kung San population
![](https://i.pinimg.com/originals/a6/0f/5d/a60f5d397767070f372be2840334e6a8.jpg)

"""

# ╔═╡ ecc59c3d-19b7-4bcf-9528-fbfb6630fc64
md"""
##### We use our new predicted function to estimate the most likely size range of a 58.0 Kg woman. 
"""

# ╔═╡ b0a5b208-17aa-4d90-b44b-378dcdaf4ccd
md"""
### Research hypothesis 2: (not a statistical hypothesis):

**A man that weights 58 Kg is much taller than a woman of the same weight**

"""

# ╔═╡ f94f4e3e-e9ca-4894-bfe6-8cc2a6fb0a48
md"""
#### Our model predictions show that a 58 Kg man is taller than a 58 Kg woman with an 85.2% certainty

### Is that significant or not?
"""

# ╔═╡ 1658fecc-9bfc-4791-96c0-ba1a3ef60d97
md"""
## Research question 2: Are men always taller than women?
"""

# ╔═╡ b0a8546a-424e-4ff7-9d60-9655349f5cda
md"""
This plot shows that men are generally taller, but some women are taller than some men. **So, what is the probability that a woman is taller than a man when we control for their weight?**
"""

# ╔═╡ 61fe41d2-88e0-4a4c-98b8-907b2849b1af
md"""
# Back to R 
### (another way to do it in R from Muenster to the world)

## The 'brms'- R package

![](https://www.simtech.uni-stuttgart.de/img/people/Paul-Buerkner.png?__scale=w:220,h:220,cx:1,cy:0,cw:397,ch:397)

Dr. Paul Bürkner
Ph.D. in Psychology from the University of Münster
"""

# ╔═╡ f49f6fce-12a0-47e7-8b22-7c94ece537f0
@rput data; # send our data to R

# ╔═╡ 00606c5c-d8ac-4177-8b54-be0848d77c4b
md"""
## Run the model in R
"""

# ╔═╡ c6c89554-5806-4b02-8905-51babe0e09f5
R"""
library(brms)
data$weight_c = log(data$weight)- log(45)
mod1 <- brm(log(height) ~ weight_c*male, family= gaussian(), data, chains = 4)
summary(mod1)
"""

# ╔═╡ 53d64366-7d32-4c75-8d98-7f5037a24193
md"""
## Extract the posteriors
"""

# ╔═╡ 8555e5fe-3aa2-4f34-a525-1802f3dab755
R"""
...
βw = post.b_weight_c
βm = post.b_male
βwm = post."b_weight_c:male"
	
R_W_58kg = p_link(58.0, 0, α, βw, post.b_male, βwm)
R_M_58kg = p_link(58.0, 1, α, βw, post.b_male, βwm)
end;

# ╔═╡ de1a1ca1-4cf5-4491-875c-30546e2f4a71
@model function Kung_San(height, weight, male, center)

	x = weight .- center

	σ ~ truncated(Cauchy(0, 2), 0, Inf) # priors for error
    α ~ Normal(4.93, 0.1) # priors for intercept
    β_w ~ Normal(0, 1) # prior for weight effect (slope)
    β_m ~ Normal(0, 1) # prior for sex effect (slope)
    β_wm ~ Normal(0, 1) # prior for the interaction effects
    μ = α .+ β_w .* x .+ β_m .* male .+ β_wm .* (x.*male) # linar model
    
	height .~ Normal.(μ, σ) # likelihood
end

# ╔═╡ 8a86d135-45b7-43b6-859b-5268d1d7f383
My_model = Kung_San(log.(data.height), log.(data.weight), data.male, log(45));

# ╔═╡ 67adc805-0830-4bd2-aa23-927c8a87ce35
# Run the model
# Multiple threads 4 
# 3000 samples
chns = sample(My_model, NUTS(), MCMCThreads(), 2000, 4);

# ╔═╡ f3e1be97-87e9-4147-8c8f-4d800991764b
chns[1:200]

# ╔═╡ ec22cf7d-0965-449b-b21d-6645aa1946fa
plot(chns)

# ╔═╡ a8bcb29c-591a-4cb3-adc5-c0f6b1fa7cb7
describe(chns)

# ╔═╡ a40887af-ea94-4739-95c5-8beec1dd7bb1
poste = DataFrame(chns);

# ╔═╡ e7f3926c-b968-44bb-b401-81ab3e751491

sum(poste.β_wm .> 0) / length(poste.β_wm)


# ╔═╡ 071c124d-ca9b-40da-9abe-d2af97c03f33
prediction_W_58kg = p_link(58.0, 0, poste.α, poste.β_w, poste.β_m, poste.β_wm)

# ╔═╡ b3fa6d23-27ac-4614-a574-aa0ae07eb476
begin
density(prediction_W_58kg, lab= "posterior distribution")
x = [mean(prediction_W_58kg), mean(prediction_W_58kg)]
CI = HDI(prediction_W_58kg; credible_mass=0.95 )
y = [0, 0.5]
plot!(x,y, lab= "mean")
x1 = [CI[1], CI[1]]
plot!(x1,y, lab= "2.5% CI")
x2 = [CI[2], CI[2]]
plot!(x2,y, lab= "97.5% CI")
end

# ╔═╡ b4bf934e-5024-4ca1-99db-d9b7d4d91f69
prediction_M_58kg = p_link(58.0, 1, poste.α, poste.β_w, poste.β_m, poste.β_wm)

# ╔═╡ 79f604ae-4e80-4e28-9ca7-ca91e2779eae
begin
density(prediction_W_58kg, lab= "Woman 58kg", legend=:topleft)
density!(prediction_M_58kg, lab= "Man 58kg", legend=:topleft)
end

# ╔═╡ 2571bc63-b739-40b7-b7ef-616da782da54
begin
ΔSex = prediction_M_58kg .- prediction_W_58kg
density(ΔSex, label= false, legend=:topleft)
CIΔSex = HDI(ΔSex; credible_mass=0.95 )
x3 = [CIΔSex[1], CIΔSex[1]]
plot!(x3,y, lab= "2.5% CI")
x4 = [CIΔSex[2], CIΔSex[2]]
plot!(x4,y, lab= "97.5% CI")	
x5 = [0, 0]
plot!(x5,y, lab= "", lw = 3, c= :black)
xlabel!("ΔSex = Man - Women")
end

# ╔═╡ 462b3b87-30d0-4ac8-84e0-bfe6266d4550
# formal test
100 * sum(prediction_M_58kg .- prediction_W_58kg .> 0) / length(prediction_M_58kg)


# ╔═╡ df5720c7-b563-4273-915e-b42022d92ba3
begin
scatter(Women.weight, Women.height, lab= "Women", legend = :topleft)
scatter!(Men.weight, Men.height, lab= "Men")
xlabel!("Weight (Kg)")
ylabel!("Height (cm)")
# estimate the height distribution of men and women from body weights that range from 30 to 70 Kg
# women
x_weight = 4:0.25:70
arrW = [p_link.(w, 0, poste.α, poste.β_w, poste.β_m, poste.β_wm) for w in x_weight]
raw_w = DataFrame(arrW, :auto) # convert the array in a Data Frame
meanW = mean.(eachcol(raw_w))
ciW = (mapcols(x -> HDI(x, credible_mass=0.95), raw_w));
lW = Vector(ciW[1,:]) .- meanW
uW = Vector(ciW[2,:]) .- meanW

# men
arrM = [p_link.(w, 1, poste.α, poste.β_w, poste.β_m, poste.β_wm) for w in x_weight]
raw_m = DataFrame(arrM, :auto) # convert the array in a Data Frame
meanM = mean.(eachcol(raw_m))
ciM = (mapcols(x -> HDI(x, credible_mass=0.95), raw_m));
lM = Vector(ciM[1,:]) .- meanM
uM = Vector(ciM[2,:]) .- meanM
plot!(x_weight, meanW,  ribbon = [lW, uW], c =:dodgerblue, linealpha= 0.3)
plot!(x_weight, meanM,  ribbon = [lM, uM], c =:red, linealpha= 0.3)
end

# ╔═╡ b884141f-dd42-42ae-bcf6-e029be55f4a8
100*sum(Matrix(raw_m .- raw_w)[:] .> 0) / length(Matrix(raw_m .- raw_w)[:])

# ╔═╡ 207c3de5-1395-431c-a06b-eb0c4652ee34
begin
density(prediction_W_58kg, lab= "Turing-Julia: Woman 58kg", legend=:topleft)
density!(prediction_M_58kg, lab= "Turing-Julia: Man 58kg", legend=:topleft)
density!(R_W_58kg, lab= "Stan-R: Woman 58kg", legend=:topleft)
density!(R_M_58kg, lab= "Stan-R: Man 58kg", legend=:topleft)
end

# ╔═╡ 5c328809-c239-42fb-8ecf-17ce52396f4c
md"""

[More on Bayesian statistics in R] (https://paul-buerkner.github.io/brms/)

[More on Bayesian statistics in Julia] (https://storopoli.io/Bayesian-Julia/)
